...
MONARCH_PARTNER_KEY=some-long-random-secret</pre>
  ...
  // Custom Pro plans are self-serve via Stripe now — no approvals to action
  // here; the table just reflects who's on one and for how much.
  ...
  body.innerHTML = countsBar + `
    <table style="width:100%;font-size:13px;border-collapse:collapse;">
      ...
      <tbody>${data.workspaces.map(w => `
        <tr style="border-top:1px solid var(--border);">
          ...
          <td>${esc(w.tier)}${w.custom_status === 'active' && w.custom_plan ? ` <span style="font-size:11px;background:#f0fdf4;border:1px solid #bbf7d0;color:#166534;border-radius:20px;padding:1px 8px;font-weight:700;">custom $${Number(w.custom_plan.monthly_usd).toFixed(2)}/mo</span>` : ''}</td>
    ...
    <p style="font-size:12px;color:var(--text-muted);margin-top:10px;">Free users grow the Monarch user base. Pro plans are self-serve — partners pay by card through Stripe, so there's nothing to approve here. Overage beyond a plan's included amounts is billed on top.</p>`;
}
